...
// ==========================================
// CEARTEE - AUTO-UPDATER (Tauri v2)
// ==========================================

async function checkForUpdates() {
  try {
    // Solo disponible en Tauri build, no en navegador dev
    ...
    if (update) {
      ...
      // Mostrar notificación al usuario (usa tu sistema de notificaciones)
      if (window.mostrarNotificacion) {
        window.mostrarNotificacion({
          titulo: 'Actualización disponible',
          mensaje: `CEARTEE ${update.version} está lista. Se instalará al cerrar.`,
          tipo: 'info',
          duracion: 8000
        });
      }
      ...
      await update.downloadAndInstall((event) => {
        switch (event.event) {
          case 'Started':
            ...
            break;
          case 'Progress':
            console.log(`⬇️ Progreso: ${event.data.chunkLength} bytes`);
            break;
          case 'Finished':
      ...
      console.log('✅ CEARTEE está actualizado');
    }
  } catch (err) {
    console.error('⚠️ Error chequeando updates:', err);
  }
}

// Chequear al inicio (modo silencioso)
document.addEventListener('DOMContentLoaded', () => {
  // Esperar 5 segundos después de cargar para no interferir con el inicio
  setTimeout(checkForUpdates, 5000);
});

// Exponer función global para botón manual en Configuración
